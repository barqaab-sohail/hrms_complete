          ...
          <div class="form-group row">
            <div class="col-md-12">
              ...
              <select name="hr_employee_id" id="hr_employee_id" class="form-control selectTwo">
                ...
                <option value="{{$employee->id}}" {{(old("hr_employee_id")==$employee->id? "selected" : "")}}>{{$employee->first_name}} {{$employee->last_name}} - {{$employee->employeeCurrentDesignation->name??''}} - {{$employee->employee_no}}</option>
                @endforeach
              </select>
