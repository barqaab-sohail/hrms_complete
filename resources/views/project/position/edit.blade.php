<div class="card-body">
	<form action="{{route('projectPosition.update',$data->id??'')}}" method="post" class="form-horizontal form-prevent-multiple-submits" enctype="multipart/form-data">
            {{csrf_field()}}
            @method('PATCH')
                ...
                <div class="col-md-6">
                    <div class="form-group row">
                        <div class="col-md-12">
                           	...
                           	<input type="text"  name="name" value="{{ old('name',$data->name??'') }}"  class="form-control" data-validation="required">
                        </div>
                    </div>
                </div>
                <!--/span-->
                    ...
                    <div class="form-group row">
                        <div class="col-md-12">
                            ...
                            <select    name="pr_position_type_id"  class="form-control selectTwo" data-validation="required">
                                    ...
                                    <option value="{{$positionType->id}}" {{(old("pr_position_type_id",$data->pr_position_type_id??'')==$positionType->id? "selected" : "")}}>{{$positionType->name}}</option>
                                    @endforeach 
                            </select>
                ...
                <div class="col-md-2">
                    <div class="form-group row">
                        <div class="col-md-12">
                           	...
                           	<input type="number" step="0.001" id="total_mm"  name="total_mm" value="{{ old('total_mm',$data->total_mm??'') }}"  class="form-control" data-validation-allowing="range[0.001;6000.00],float"> 
                        </div>
                    </div>
                </div>
                  <!--/span-->
