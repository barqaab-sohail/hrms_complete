...
<div class="card">
	<div class="card-body">	
		...
		<hr>
		<form id="inputSearch" method="get" class="form-horizontal form-prevent-multiple-submits" action="{{route('input.result')}}" enctype="multipart/form-data">
            {{csrf_field()}}
            <div class="form-body">
            	<div class="row" >
		            <div class="col-md-4">
		                ...
		                <div class="form-group row">
		                    <div class="col-md-12 required">
		                   		<label class="control-label text-right">Name of Month</label><br>
		                   		<select  name="month"  id="month" class="form-control searchSelect">
		                            <option value=""></option>
		                            @foreach($months as $month)
									<option value="{{$month->id}}" {{(old("month")==$month->id? "selected" : "")}}>{{$month->month}} {{$month->year}}</option>
		                            @endforeach
		                        </select>    
		                    </div>
		                </div>
