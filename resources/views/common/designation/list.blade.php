...
@section('Heading')
<h3 class="text-themecolor">Designations List</h3>
@stop
@section('content')
<!-- Model -->
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<form id="designationForm" name="designationForm" action="{{route('hrDesignation.store')}}" class="form-horizontal">
					<input type="hidden" name="designation_id" id="designation_id">
					<div class="row">
						<div class="col-md-7">
							<div class="form-group">
								<label class="control-label text-right">Designation Name<span class="text_requried">*</span></label>
...
<div class="card">
	<div class="card-body">
		<button type="button" class="btn btn-success float-right" id="createDesignation" data-toggle="modal">Add Designation</button>
		<h4 class="card-title" style="color:black">List of Designations</h4>
...
<script>
	$(document).ready(function() {
		...
		$(function() {
			...
			$('#createDesignation').click(function(e) {
				$('#designation_id').val('');
				$('#name').val('');
				$('#level').val('').change();
			...
			$('body').unbind().on('click', '.editDesignation', function() {
				var designation_id = $(this).data('id');
				$.get("{{ url('hrms/misc/hrDesignation') }}" + '/' + designation_id + '/edit', function(data) {
			...
			$('body').on('click', '.deleteDesignation', function() {
				...
				if (con) {
					$.ajax({
						type: "DELETE",
						url: "{{ route('hrDesignation.store') }}" + '/' + designation_id,
						success: function(data) {
							table.draw();
							if (data.error) {
