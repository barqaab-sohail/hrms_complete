...
<script type="text/javascript">
    ...
    $(function() {
        ...
        $('body').unbind().on('click', '.editDocument', function() {
            var photocopy_document_id = $(this).data('id');
            $("#formDocument").attr("method", "PUT");
            $.get("{{ url('photocopy_documents') }}" + '/' + photocopy_document_id + '/edit', function(data) {
                $('#formDocument').show();
                ...
                $('#photocopy_document_id').val(data.id);
        ...
        $('body').on('click', '.deleteDocument', function() {
            ...
            if (con) {
                $.ajax({
                    type: "DELETE",
                    url: "{{ route('photocopy_documents.store') }}" + '/' + photocopy_document_id,
                    success: function(data) {
                        table.draw();
                        $('#json_message').html('<div id="json_message" class="alert alert-success" align="left"><a href="#" class="close" data-dismiss="alert">&times;</a><strong>' + data.success + '</strong></div>');
                        if (data.error) {
