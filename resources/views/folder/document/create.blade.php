...
<div class="row justify-content-end">
    <div class="col-lg-12 reducedCol">
        <div class="card">
            <div class="row">
                <div class="col-md-9">
                    <h2 class="card-title m-4" style="color:black">{{$folder->name}} Documents</h2>
                </div>
                <div class="col-md-3">
                            ...
                            <div class="col-md-8">
                                <div class="form-group row">
                                   <div class="col-md-12">
                                        ...
                                        <input type="hidden" name="folder_document_id" id="folder_document_id">
...
<script type="text/javascript">
    ...
    $(function() {
        ...
        var table = $('.data-table').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "{{ route('folder_documents.create') }}",
                data: {
                    folderId: "{{$folder->id}}"
                }
            },
        ...
        $('body').unbind().on('click', '.editDocument', function() {
            var folder_document_id = $(this).data('id');
            $("#formDocument").attr("method", "PUT");
            $.get("{{ url('folder_documents') }}" + '/' + folder_document_id + '/edit', function(data) {
                $('#formDocument').show();
        ...
        $("#formDocument").submit(function(e) {
            ...
            $.ajax({
                data: formData,
                url: "{{ route('folder_documents.store') }}",
                type: "POST",
                //dataType: 'json',
                contentType: false,
                cache: false,
        ...
        $('body').on('click', '.deleteDocument', function() {
            ...
            if (con) {
                $.ajax({
                    type: "DELETE",
                    url: "{{ route('folder_documents.store') }}" + '/' + folder_document_id,
                    success: function(data) {
                        table.draw();
                        $('#json_message').html('<div id="json_message" class="alert alert-success" align="left"><a href="#" class="close" data-dismiss="alert">&times;</a><strong>' + data.success + '</strong></div>');
                        if (data.error) {
