    ...
    public function result(Request $request)
    {
        ...
        if ($request->filled('document_name')) {
            ...
            $result = HrEmployee::whereIn('hr_status_id', [1, 2, 3, 4, 5, 6, 7])
                ...
                ->when($data['category'], function ($query) use ($data) {
                    return  $query->join('employee_categories', 'employee_categories.hr_employee_id', '=', 'hr_employees.id')->orderBy('employee_categories.effective_date', 'desc')->where('employee_categories.hr_category_id', $data['category'])->groupBy('employee_categories.hr_employee_id');
                })
                ->when($data['designation'], function ($query) use ($data) {
