...
class PromotionController extends Controller
{
        ...
        if ($request->ajax()) {
          ...
          return  DataTables::of($data)
                 ...
                 ->addColumn('Edit', function($row){
                         ...
                         $btn = '<a href="javascript:void(0)" data-toggle="tooltip"  data-id="'.$row->id.'" data-original-title="Edit" class="edit btn btn-primary btn-sm editPromotion">Edit</a>';
                                           
                          return $btn;
                  })
                  ->addColumn('Delete', function($row){                
                      
                         $btn = '<a href="javascript:void(0)" data-toggle="tooltip"  data-id="'.$row->id.'" data-original-title="Delete" class="btn btn-danger btn-sm deletePromotion">Delete</a>';
    ...
    public function store(PromotionStore $request){
        ...
        DB::transaction(function () use ($request, $input, $employeeFullName) {  
            ...
            if ($request->filled('hr_designation_id')) {
                $promotionDesignation = PromotionDesignation::where('hr_promotion_id', $input['promotion_id'])->first();
            ...
            if ($request->filled('hr_department_id')) {
                $promotionDepartment = PromotionDepartment::where('hr_promotion_id', $input['promotion_id'])->first();
            ...
            if ($request->filled('hr_salary_id')) {
                $promotionSalary = PromotionSalary::where('hr_promotion_id', $input['promotion_id'])->first();
            ...
            if ($request->filled('hr_manager_id')) {
                $promotionManager = PromotionManager::where('hr_promotion_id', $input['promotion_id'])->first();
            ...
            if ($request->filled('hr_grade_id')) {
                $promotionGrade = PromotionGrade::where('hr_promotion_id', $input['promotion_id'])->first();
            ...
            if ($request->filled('hr_category_id')) {
                $promotionCategory = PromotionCategory::where('hr_promotion_id', $input['promotion_id'])->first();
    ...
    //         $input['effective_date'] = \Carbon\Carbon::parse($request->effective_date)->format('Y-m-d');
    //     } else {
    //         $data = HrPromotion::find(session('promotion_edit_id'));
    //         if ($data) {
