...
class MonthlyProgressController extends Controller
{
    ...
    public function show($prDetailId)
    {
        ...
        $progressActivities = PrProgressActivity::where('pr_detail_id', $prDetailId)->where('weightage', ">", 0)->get();

        $prMonthlyProgress = PrMonthlyProgress::where('pr_detail_id', $prDetailId)->get();
        $prDetail = PrDetail::find($prDetailId);

        $view =  view('project.progress.monthly.create', compact('prMonthlyProgress', 'progressActivities', 'prDetail'))->render();
    ...
    public function create(Request $request)
    {
        ...
        if ($request->ajax()) {
            $data = PrMonthlyProgress::where('pr_detail_id', $request->prDetailId)->latest()->get();
    ...
    public function store(MonthlyProgressStore $request)
    {
        ...
        DB::transaction(function () use ($input, $request) {
            ...
            PrMonthlyProgress::updateOrCreate(
                ['id' => $input['monthlyProgress_id']],
                [
