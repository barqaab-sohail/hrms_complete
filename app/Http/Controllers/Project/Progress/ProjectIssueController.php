...
class ProjectIssueController extends Controller
{
    ...
    public function show($prDetailId)
    {
        ...
        $prIssues = PrIssue::where('pr_detail_id', $prDetailId)->get();
        $prDetail = PrDetail::find($prDetailId);
        $view =  view('project.progress.issues.create', compact('prIssues', 'prDetail'))->render();
    ...
    public function create(Request $request)
    {
        ...
        if ($request->ajax()) {
            $data = PrIssue::where('pr_detail_id', $request->prDetailId)->latest()->get();
