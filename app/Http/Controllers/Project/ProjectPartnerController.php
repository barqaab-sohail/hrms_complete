...
class ProjectPartnerController extends Controller
{
    public function show($prDetailId)
    {
        ...
        $view =  view('project.partner.create', compact('prRoles', 'partners', 'prDetail'))->render();
    ...
    public function create(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $data =  PrPartner::where('pr_detail_id', $request->prDetailId)->get();
    ...
    public function store(PrPartnerStore $request)
    {
        ...
        DB::transaction(function () use ($input, $request) {
            PrPartner::updateOrCreate(
                ['id' => $request->pr_partner_id],
                $input
            );
        }); // end transcation
