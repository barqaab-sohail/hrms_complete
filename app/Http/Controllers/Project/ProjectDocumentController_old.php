...
class ProjectDocumentController extends Controller
{
    ...
    {
        $query = PrDocument::where('pr_detail_id', $prDetailId)
        ...
        if ($request->has('search_date') && !empty($request->search_date)) {
            //$search_date = \Carbon\Carbon::parse($request->search_date)->format('Y-m-d');
            $query->where('document_date', $request->search_date);
        }
        ...
        $data = $query->orderByRaw('ISNULL(document_date), document_date desc')->get();
    ...
    public function create(Request $request)
    {
        ...
        if ($request->ajax()) {
            $query = PrDocument::where('pr_detail_id', $request->prDetailId);
